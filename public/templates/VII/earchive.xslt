        ...
        <div class="invoice">
            ...
            <tbody>
              <tr>
                ...
                <td width="30%" class="header__stamp">
                  <!-- GIB LOGO -->
                  <img style="width:91px;" align="middle" alt="e-Arşiv Fatura Logo"
                  ...
                  <h3 align="center">e-Arşiv Fatura</h3>
                  <!-- SIGNATURE -->
                  {{IMZA}}
                </td>
                <td width="35%">
          ...
          <div id="eArsivAlan" style="margin-top:10px">
            <div id="irsaliye" style="margin-bottom:10px">
              ...
              <xsl:for-each
                select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'IS_DESPATCH']"
                > İrsaliye yerine geçer </xsl:for-each>

            </div>


            <xsl:for-each
              select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'IS_INTERNET']"
              > Bu satış internet üzerinden yapılmıştır <div id="internetSatis"
                style="margin-top:5px; font-size:10px;">
                <table class="intSatisBilgi" style="border:1px solid #ccc; width:670px" id="info">
                  <tbody>
                    <tr>
                      <td style="font-weight: bold; border-bottom:1px solid #ccc"> &#160;&#160;Web
                        Adresi </td>
                      <td style="border-bottom:1px solid #ccc">
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_WEBSITE']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                      <td style="font-weight: bold; border-bottom:1px solid #ccc"> Ödeme Şekli </td>
                      <td style="border-bottom:1px solid #ccc">
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_PAYMENT_METHOD']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                      <td style="font-weight: bold; border-bottom:1px solid #ccc"> Ödeme Tarihi </td>
                      <td style="border-bottom:1px solid #ccc">
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_PAYMENTDATE']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                    </tr>
                    <tr>
                      <td style="font-weight: bold;"> &#160;&#160;Gönderiyi Taşıyan </td>
                      <td>
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_TRANSPORTER']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                      <td style="font-weight: bold;"> Taşıyıcı VKN/TCKN </td>
                      <td>
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_TRANSPORTER_REGISTER_NUMBER']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                      <td style="font-weight: bold;"> Gönderim Tarihi </td>
                      <td>
                        <xsl:for-each
                          select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'INT_TRANSPORT_DATE']">
                          <xsl:value-of select="cbc:ID"/>
                        </xsl:for-each>
                      </td>
                    </tr>
                ...
                <table id="info" class="intSatisBilgi"
                  style="border:1px solid #ccc;  margin-top:10px; margin-bottom:10px; width:670px">
                  <tbody>
                    <tr style="text-align:center">
                      <td colspan="5" style="font-weight:bold">İade Edilen Mal Bilgileri</td>
                    </tr>
                    <tr style="border:0px">
                      <td class="iadeBorderTop" style="width:150px; text-align:center">Stok
                        Kodu</td>
                      ...
                      <td class="iadeBorderTop" style="width:100px; text-align:center">Birim
                        Fiyatı</td>
                      <td class="iadeBorderTop" style="width:100px; text-align:center">Tutarı</td>
                    </tr>
                    <xsl:for-each select="//n1:Invoice/cac:InvoiceLine">
                      <tr>
                        <td class="iadeBorderTop" style="height:18px; border-bottom:0px"/>
                        <td class="iadeBorderLeft"/>
                        <td class="iadeBorderLeft"/>
                        <td class="iadeBorderLeft"/>
                        <td class="iadeBorderLeft"/>
                      </tr>
            ...
            <div id="elektronikOrtam" style="width:800px">
              ...
              <xsl:for-each
                select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentTypeCode = 'SEND_TYPE']">
                <xsl:if test="cbc:ID = 'ELEKTRONIK'">
                  <div style="width:auto;"> e-Arşiv izni kapsamında elektronik ortamda iletilmiştir.
                  </div>
                </xsl:if>
              </xsl:for-each>
            </div>
            ...
            <div id="elektronikOrtamDogrula" style="width:800px">
              <div style="width:auto; margin-bottom:10px; margin-top:3px"> Bu faturayı elektronik
                olarak görmek ve doğrulamak için <a target="_blank" style="font-weight:bold"
                  href="http://e-fatura.link">
                  <u>e-Arşiv Fatura.link</u>
                </a> adresini ziyaret ediniz. </div>
            </div>
          </div>
