...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
			...
			<body
						...
						<tbody>
							<tr valign="top">
								<td width="40%">
								    ...
								    <b>DÜZENLEYEN</b><br></br>
									<table align="center" border="0" width="100%">
										<tbody>
											
												<xsl:for-each select="NESVoucher/CompanyInfo">
													<tr align="left">
													<td align="left">
								...
								<td width="40%" style="vertical-align:middle;text-align:  center;">
									...
									<div id="qrvalue" style="visibility: hidden"><xsl:value-of select="NESVoucher/VoucherGeneralInfo/UUID"/></div>
									<script type="text/javascript">
									...
									<table class="despatchTable" style="float:right;">
										<tbody>
											<tr>
												...
												<td>
													<xsl:for-each select="NESVoucher/VoucherGeneralInfo/VoucherSerieOrNumber">
														<xsl:apply-templates/>
													</xsl:for-each>
												</td>
											</tr>
											<tr>
												...
												<td>
													<xsl:for-each select="NESVoucher/VoucherGeneralInfo/IssueDate">
											...
											<tr>
												...
												<td >
													<xsl:for-each select="NESVoucher/VoucherGeneralInfo/IssueTime">
														<xsl:apply-templates select="."/>
													</xsl:for-each>
												</td>
											</tr>																				
			    ...
			    <div style="width:800px">
			        <table class="totalTable" width="800px">
			        	...
			        	<xsl:variable name="brutTotal" select="sum(for $i in NESVoucher/VoucherLines/VoucherLine return $i/Price)"/>
						<xsl:variable name="genelToplam" select="sum(for $i in NESVoucher/VoucherLines/VoucherLine return $i/NetAmount)"/>
			        	<xsl:variable name="gvStopajTotal" select="sum(for $i in NESVoucher/VoucherLines/VoucherLine return (round(($i/Price * $i/GVWithholdingPercent)) div 100))"/>
			        	<xsl:variable name="kdvTotal" select="sum(for $i in NESVoucher/VoucherLines/VoucherLine return (round(($i/Price * $i/KDVPercent)) div 100))"/>
			        	<xsl:variable name="kdvTevkifatTotal" select="sum(for $i in NESVoucher/VoucherLines/VoucherLine return ((round(($i/Price * $i/KDVPercent)) div 100) * $i/KDVWithholdingPercent) div 100)"/>
			            ...
			            <tr>
			                ...
			                <td>
			                	...
			                	<xsl:if test="//NESVoucher/VoucherGeneralInfo/CurrencyCode">
			                		<xsl:text> </xsl:text>
			                		<xsl:if test="//NESVoucher/VoucherGeneralInfo/CurrencyCode = &quot;TRL&quot; or //NESVoucher/VoucherGeneralInfo/CurrencyCode = &quot;TRY&quot;">
			                			<xsl:text>TL</xsl:text>
			                		</xsl:if>
			                		<xsl:if test="//NESVoucher/VoucherGeneralInfo/CurrencyCode != &quot;TRL&quot; and //NESVoucher/VoucherGeneralInfo/CurrencyCode != &quot;TRY&quot;">
			        		...
			        		<td>
                   				<xsl:choose>
         							<xsl:when test="$genelToplam > 0">
          							<xsl:value-of select="format-number($genelToplam, '###.##0,00', 'european') "/>
         							</xsl:when>
         						<xsl:otherwise>
	...
	<xsl:template match="//NESVoucher/VoucherLines/VoucherLine">
		...
		<xsl:variable name="kdvTevkifatTotal" select="round(($kdvTotal * ./KDVWithholdingPercent)) div 100"/>
		...
		<tr>
			...
			<td>
				...
				<xsl:choose>
         			...
         			<xsl:otherwise>
         				<xsl:value-of select="format-number(($netTotal + $kdvTotal) -  $kdvTevkifatTotal, '###.##0,00', 'european') "/>
         			</xsl:otherwise>
       			</xsl:choose>
